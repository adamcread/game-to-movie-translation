...
class CocoFilter():
    ...
    def _filter_annotations(self):
        ...
        for image_id, segmentation_list in self.segmentations.items():
            # for each segmentation for image
            for segmentation in segmentation_list:
                original_seg_cat = segmentation['category_id']
                image_name = self.images[segmentation["image_id"]]['file_name']
    ...
    def main(self, args):
        ...
        # self.filter_list = list(map(lambda x: x.rstrip(), open(args.filter_list, 'r').readlines()))
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-f', '--filter_list', dest='filter_list',
    #     help="text file containing list of files to keep line by line")
